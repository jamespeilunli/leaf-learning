...
import { buildLaunchGraph, buildLaunchGraphLinks, type LaunchGraphLink, type LaunchGraphNode } from './launchGraph'
  ...
  useEffect(() => {
    ...
    const simulation = forceSimulation(nodes)
      .force(
        ...
        forceLink<SimNode, SimLink>(links)
          .id((node) => node.id)
          .distance((link) => (link.source.parentId === link.target.id || link.target.parentId === link.source.id ? 44 : 62))
          .strength((link) => (link.source.parentId === link.target.id || link.target.parentId === link.source.id ? 0.9 : 0.34)),
      )
      .force('charge', forceManyBody().strength(-72))
      .force('center', forceCenter(0, 0))
      .force('collision', forceCollide<SimNode>().radius((node) => node.size + 8).strength(0.72))
      .velocityDecay(0.16)
      .alphaDecay(0.008)
    ...
    const draw = (timestamp: number) => {
      ...
      for (const link of links) {
        ...
        const sourceDriftX = Math.cos(timestamp * 0.00055 + source.driftOffset) * 1.4
        const sourceDriftY = Math.sin(timestamp * 0.00045 + source.driftOffset) * 1.4
        const targetDriftX = Math.cos(timestamp * 0.00055 + target.driftOffset) * 1.4
        const targetDriftY = Math.sin(timestamp * 0.00045 + target.driftOffset) * 1.4
        const isParentLink =
          source.parentId === target.id || target.parentId === source.id

        context.beginPath()
        context.moveTo(width / 2 + source.x + sourceDriftX, height / 2 + source.y + sourceDriftY)
        context.lineTo(width / 2 + target.x + targetDriftX, height / 2 + target.y + targetDriftY)
      ...
      for (const node of nodes) {
        ...
        const x = width / 2 + node.x + driftX
        const y = height / 2 + node.y + driftY
