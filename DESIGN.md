...
All Phase 2 learning flows use technical depth by default. Resource selection, prerequisite explanations, and inline chat should favor formal definitions, mechanisms, derivations, and precise terminology.
...
Phase 2 begins with the chosen goal node at the top of a tree. The AI searches for the best technical resource explaining this topic, reads it, and generates a fully expanded node: a resource link, a description of what the resource covers, and a set of grayed-out prerequisite nodes representing topics the resource assumes the reader knows.
...
- **"Don't know"** — fully expands the node: finds a resource, generates a description, and creates its own set of grayed-out prerequisite children.
...
Web search enabled. Given a node label, known_topics list, and the goal label. Returns: a resource (URL, title, description) and a list of prerequisite topics detected from the resource (each with a label and a 1-sentence hint). The known_topics list must be passed so already-known prerequisites are excluded.

### Explain More

No web search. Given a grayed node's label and its parent node's label and description. Returns a technical explanation of what the prerequisite is and why it matters in the context of the parent topic. Approximately 3–5 sentences.

### Inline Chat

No web search. System context: node label, description, resource description, technical depth, path from goal to this node. Streaming. Responds in technical tutor style.
