...
def normalize_phase2_graph(session: Session) -> None:
    ...
    node_order = {node_id: index for index, node_id in enumerate(session.nodes.keys())}
    ...
    for node_ids in grouped.values():
        ...
        def priority(node_id: str) -> tuple[int, int, int]:
            ...
            return (
                ...
                0 if node_id == session.focus_node_id else 1,
            )
        ...
        for duplicate_id in node_ids:
            ...
            if canonical.why_interesting is None:
            ...
            for node in session.nodes.values():
                node.child_ids = [canonical_id if child_id == duplicate_id else child_id for child_id in node.child_ids]
    ...
    for node in session.nodes.values():
        if node.phase != "2" or node.id == session.focus_node_id:
            continue
        parents = [parent_id for parent_id in incoming.get(node.id, []) if parent_id in session.nodes]
        if not parents:
            node.parent_id = None
            continue
        parents.sort(key=lambda parent_id: (session.nodes[parent_id].depth, session.nodes[parent_id].label))
        node.parent_id = parents[0]


def upsert_phase2_edge(session: Session, from_id: str, to_id: str, label: str | None = "requires") -> GraphEdge:
    for edge in session.edges:
...
def find_phase2_node_by_label(session: Session, label: str) -> GraphNode | None:
    ...
    for node in session.nodes.values():
        ...
        current_rank = (node.depth, node_order.get(node.id, 10**9))
        best_rank = (best_match.depth, node_order.get(best_match.id, 10**9))
